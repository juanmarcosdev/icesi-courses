...
**Actividad A — Recursión lineal:**
Implementa `suma(x: Int, y: Int): Int` usando `suc` y `pred`.
...
**Actividad B — Recursión de cola:**
Transforma la función a `sumaRCola(x: Int, y: Int): Int`.
**No se necesita un parámetro extra:** `y` ya puede actuar como acumulador.
Pista: ¿qué pasa si en vez de `suc(sumaRCola(..., y))` escribes `sumaRCola(..., suc(y))`?

```scala
import scala.annotation.tailrec

@tailrec
def sumaRCola(x: Int, y: Int): Int =
  x match {
    case 0 => y
    case _ => ???   // mueve suc al interior del argumento y
  }

// Invocación: sumaRCola(x, y)
```
...
def sumaRCola(x: Int, y: Int): Int =
  x match {
    case 0 => y
    case _ => sumaRCola(pred(x), suc(y))
  }

// Invocación: sumaRCola(x, y)  — misma firma, sin parámetro extra.
// y absorbe un suc en cada llamado en vez de acumularlo por fuera.
// Traza sumaRCola(3, 2):
//   → sumaRCola(2, 3)
//   → sumaRCola(1, 4)
//   → sumaRCola(0, 5) → 5  ✓
